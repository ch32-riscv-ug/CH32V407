...
ENUM_START:
    ...
    if( s == ERR_SUCCESS )
    {
        memcpy(MyDevDescr, DevDesc_Buf, DEF_DEVICE_DESC_LEN);
...
GETREP_START:
            ...
            if( s == ERR_SUCCESS )
            {
                KeyRepDesc_Len[num] = HostCtl[ index ].Interface[ num ].HidDescLen;
                memcpy(KeyRepDesc[num],Com_Buf, KeyRepDesc_Len[num]);
...
uint8_t USBH_EnumHidDevice( uint8_t usb_port, uint8_t index, uint8_t ep0_size )
{
    ...
    DUG_PRINTF("Get StringDesc0: ");
    s = USBH_GetStrDescr( usb_port, ep0_size, 0);
    if( s == ERR_SUCCESS )
    {
        memcpy(MyLangDescr, Com_Buf, Com_Buf[0]);
    ...
    if( DevDesc_Buf[ 14 ] )
    {
        ...
        if( s == ERR_SUCCESS )
        {
            memcpy(MyManuInfo, Com_Buf, Com_Buf[0]);
            /* Print USB vendor string descriptor */
#if DEF_DEBUG_PRINTF
            for( i = 0; i < Com_Buf[ 0 ]; i++ )
            {
    ...
    if( DevDesc_Buf[ 15 ] )
    {
        ...
        if( s == ERR_SUCCESS )
        {
            memcpy(MyProdInfo, Com_Buf, Com_Buf[0]);
    ...
    if( DevDesc_Buf[ 16 ] )
    {
        ...
        if( s == ERR_SUCCESS )
        {
            memcpy(MySerNumInfo, Com_Buf, Com_Buf[0]);
