 ...
 * @return  none
 */
uint8_t HID_SetFeature( uint8_t ep0_size, uint8_t intf_num, uint8_t duration, uint8_t reportid )
{
    memcpy( pUSBHS_SetupRequest, SetupSetFeature, sizeof( USB_SETUP_REQ ) );
    pUSBHS_SetupRequest->wValue = ( (uint16_t)duration << 8 ) | reportid;
    pUSBHS_SetupRequest->wIndex = (uint16_t)intf_num;
 ...
 * @return  none
 */
uint8_t HID_ClearFeature( uint8_t ep0_size, uint8_t intf_num, uint8_t duration, uint8_t reportid )
{
